-- ============================================================================
-- MTDRB Admin - Complete Database Schema v2.0
-- ============================================================================
-- 
-- This is a COMPLETE, AUDITED SQL script that sets up the entire database.
-- Based on comprehensive audit of application code and TypeScript interfaces.
-- Run this single file in your Supabase SQL Editor.
--
-- Includes:
-- - All 18 tables with complete field definitions
-- - All RLS policies for multi-tenancy security
-- - All RPC functions for analytics and reporting
-- - All triggers and optimized indexes
-- - Addresses all issues identified in SCHEMA_AUDIT_REPORT.md
--
-- ============================================================================
...
DROP FUNCTION IF EXISTS get_member_full_name(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_trainer_full_name(UUID) CASCADE;
...
-- PLANS TABLE (membership plans) - Created before members since members references it
...
-- MEMBERS TABLE (Enhanced with all fields from TypeScript interface)
CREATE TABLE IF NOT EXISTS members (
  ...
  -- Health & Fitness Profile (stored in metadata for flexibility)
  -- height, weight, target_weight, fitness_level, medical_conditions, injuries, etc.
...
CREATE INDEX IF NOT EXISTS idx_members_tenant_status_membership ON members(tenant_id, status, membership_status);
...
CREATE INDEX IF NOT EXISTS idx_classes_tenant_start_status ON classes(tenant_id, start_time, status);
...
CREATE INDEX IF NOT EXISTS idx_class_bookings_class_status ON class_bookings(class_id, status);
...
-- INVOICES TABLE (Enhanced with all fields)
...
-- EXPENSES TABLE (Enhanced with all fields)
CREATE TABLE IF NOT EXISTS expenses (
  ...
  category TEXT,
  date DATE NOT NULL,
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected', 'cancelled', 'paid')),
  payment_method TEXT,
  vendor TEXT,
  receipt_url TEXT,
...
-- Add missing columns to existing expenses table if they don't exist
DO $$ 
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
      ...
      -- Copy description to title for existing records, use 'Expense' as fallback
      UPDATE expenses SET title = COALESCE(description, 'Expense') WHERE title IS NULL;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'expenses' AND column_name = 'updated_by') THEN
      ALTER TABLE expenses ADD COLUMN updated_by UUID REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'expenses' AND column_name = 'internal_notes') THEN
      ALTER TABLE expenses ADD COLUMN internal_notes TEXT;
    END IF;

    -- Add public_notes column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'expenses' AND column_name = 'public_notes') THEN
      ALTER TABLE expenses ADD COLUMN public_notes TEXT;
    END IF;

    -- Add country_code column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'expenses' AND column_name = 'country_code') THEN
      ALTER TABLE expenses ADD COLUMN country_code TEXT;
    END IF;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_activities_tenant_type_created ON activities(tenant_id, type, created_at DESC);
...
CREATE OR REPLACE FUNCTION get_member_full_name(p_member_id UUID)
RETURNS TEXT AS $$
...
GRANT EXECUTE ON FUNCTION get_member_full_name(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_trainer_full_name(UUID) TO authenticated;
