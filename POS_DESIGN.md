...
The exposed production-preview URL served the SPA shell for `/dashboard/pos`, but the connected browser showed no rendered content after waiting. The local headless runtime smoke test on `127.0.0.1` did render the POS workspace successfully. This preview discrepancy is being investigated as an environment/auth or stale-preview-process issue rather than accepted as a successful production-browser check.

After rebuilding with the Supabase variables embedded at build time, the exposed production preview rendered the public landing page successfully. Navigating to `/dashboard/pos` without an authenticated session redirected visibly to `/login` and rendered the complete sign-in form. The earlier blank state was caused by the local preview being overwritten by a no-environment build during validation, not by the POS route.
