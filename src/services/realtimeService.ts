...
export interface RealtimeNotification {
  id: string;
  type: 'member_checkin' | 'class_capacity_alert' | 'new_assignment' | 'urgent_task' | 'payment_received';
  title: string;
  message: string;
  targetRoles: string[];
  targetUsers?: string[];
...
class RealtimeService {
  ...
  private notificationCallbacks: Map<string, (notification: RealtimeNotification) => void> = new Map();
  ...
  subscribeToNotifications(
    ...
    const channel = supabase.channel(`notifications_${tenantId}_${userRole}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
    ...
    this.notificationCallbacks.set(`notifications_${tenantId}_${userRole}`, callback);
  }
  ...
  subscribeToClassCapacityAlerts(
    ...
    const channel = supabase.channel(`class_capacity_${tenantId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          ...
          const utilizationPercent = (updatedClass.current_bookings / updatedClass.capacity) * 100;
  ...
  subscribeToMemberCheckins(
    ...
    const channel = supabase.channel(`checkins_${tenantId}_${trainerId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        async (payload) => {
          ...
          if (booking.status === 'checked_in' && payload.old.status !== 'checked_in') {
  ...
  async sendTargetedNotification(notification: Omit<RealtimeNotification, 'id' | 'created_at'>) {
    try {
  ...
  async sendClassCapacityAlert(classId: string, tenantId: string) {
    ...
    if (classData) {
      const utilizationPercent = (classData.current_bookings / classData.capacity) * 100;
  ...
  private getNotificationIcon(type: string): string {
    switch (type) {
      case 'member_checkin': return '✅';
      case 'class_capacity_alert': return '⚠️';
  ...
  async getLiveMemberCount(tenantId: string): Promise<number> {
    try {
      ...
      const uniqueMembers = new Set(data?.map(booking => booking.member_id) || []);
