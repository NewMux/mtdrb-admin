...
const ERROR_MESSAGES: Record<string, Record<string, string>> = {
  [ErrorContext.MEMBER_MANAGEMENT]: {
    ...
    'foreign_key_violation': 'Cannot delete this member as they have active bookings or invoices',
  ...
  [ErrorContext.TRAINER_MANAGEMENT]: {
    ...
    'trainer_has_active_classes': 'Trainer has active classes and cannot be removed'
  },
...
class ErrorHandler {
  ...
  private handlePostgrestError(error: PostgrestError, context: ErrorContext): ProcessedError {
    const errorCode = this.mapPostgresErrorCode(error.code, error.message);
    const contextMessages = ERROR_MESSAGES[context] || {};
    const userMessage = contextMessages[errorCode] || 
  ...
  private handleNetworkError(error: Error, context: ErrorContext): ProcessedError {
    return {
      userMessage: ERROR_MESSAGES[context]?.network_error || DEFAULT_MESSAGES.network_error,
      technicalMessage: error.message,
      code: 'NETWORK_ERROR',
      context,
      shouldLog: true,
  ...
  private handleGenericError(error: Error, context: ErrorContext): ProcessedError {
  ...
  private handleUnknownError(error: unknown, context: ErrorContext): ProcessedError {
    return {
