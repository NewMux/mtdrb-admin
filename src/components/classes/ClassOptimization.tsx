...
export default function ClassOptimization(_props: Props) {
  const calculateOptimalSchedule = (): { recommendations: ScheduleRecommendation[] } => {
  ...
  const calculateCapacityOptimization = (): { recommendations: CapacityRecommendation[] } => {
  ...
  const calculateWaitlistInsights = (): { recommendations: WaitlistRecommendation[] } => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h2 className="text-xl font-semibold text-blue-900 mb-4">AI-Powered Optimization Insights</h2>
        <p className="text-blue-700">
          Our AI has analyzed your class data and member behavior to provide the following recommendations
          for optimizing your class schedule and capacity.
        </p>
      </div>
            ...
            {scheduleOptimization.recommendations.map((rec) => (
              <div key={rec.id} className="border rounded-lg p-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-medium text-gray-900">{rec.className}</h4>
                    <p className="text-sm text-gray-500 mt-1">Current: {rec.currentTime}</p>
                    <p className="text-sm text-green-600 mt-1">Suggested: {rec.suggestedTime}</p>
                  </div>
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Capacity Optimization</h3>
          </div>
          <div className="space-y-4">
            {capacityOptimization.recommendations.map((rec) => (
              <div key={rec.id} className="border rounded-lg p-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-medium text-gray-900">{rec.className}</h4>
                    <p className="text-sm text-gray-500 mt-1">Current Capacity: {rec.currentCapacity}</p>
                    <p className="text-sm text-green-600 mt-1">Suggested Capacity: {rec.suggestedCapacity}</p>
                  </div>
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Waitlist Insights</h3>
          </div>
          <div className="space-y-4">
            {waitlistInsights.recommendations.map((rec) => (
              <div key={rec.id} className="border rounded-lg p-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-medium text-gray-900">{rec.className}</h4>
                    <p className="text-sm text-gray-500 mt-1">Average Waitlist: {rec.averageWaitlist} members</p>
                    <p className="text-sm text-blue-600 mt-1">{rec.suggestion}</p>
                  </div>
        ...
        <div className="p-6">
          ...
          <ul className="space-y-2">
            <li className="flex items-center gap-2 text-gray-700">
              ...
              Review and implement schedule changes for Yoga Flow and HIIT Training
            </li>
            <li className="flex items-center gap-2 text-gray-700">
            ...
            <li className="flex items-center gap-2 text-gray-700">
              ...
              Evaluate adding additional CrossFit session based on waitlist demand
            </li>
          </ul>
        </div>
      </div>
