...
import { 
  FiX, FiCalendar, FiClock, FiUsers, FiDollarSign, FiUser, FiMapPin,
  FiTag, FiFileText, FiRepeat, FiSave, FiAlertCircle, FiSettings,
  FiInfo, FiZap, FiTarget, FiHome, FiActivity, FiSkipBack
} from 'react-icons/fi';
...
const schema = z.object({
  ...
  capacity: z.coerce.number().min(1, 'Capacity must be at least 1').max(200, 'Capacity cannot exceed 200'),
...
const classTypes = [
  ...
  { 
    ...
    suggestedCapacity: { min: 6, max: 15 },
    suggestedDuration: 60,
    equipment: ['Olympic bars', 'Plates', 'Kettlebells', 'Pull-up bars', 'Boxes'],
...
export default function ClassFormModal({ isOpen, onClose, onSaved, mode = 'add', classData }: ClassFormModalProps) {
  ...
  const createRecurringClasses = async (baseClass: any, formData: FormData): Promise<number> => {
    try {
      const recurringClasses = [];
      const baseDate = new Date(formData.date);
      const endDate = formData.recurring_end_date ? new Date(formData.recurring_end_date) : null;
              ...
              >
                ...
                {trainers.map(trainer => (
                  <option key={trainer.id} value={trainer.id}>
                    ...
                    {trainer.specialties && ` (${trainer.specialties.slice(0, 2).join(', ')})`}
                  </option>
                ))}
              </select>
            </div>
