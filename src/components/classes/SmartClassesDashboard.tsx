...
export default function SmartClassesDashboard({
  refreshKey,
}: SmartClassesDashboardProps) {
    ...
    try {
      ...
      if (todayClasses === 0) {
        smartInsights.push({
          ...
          description:
            "No classes scheduled for today. Consider adding more sessions to maximize revenue.",
      ...
      setInsights({
        ...
        activeClasses,
        todayClasses,
        weeklyClasses: classes.filter((c) => new Date(c.starttime) >= weekAgo)
          .length,
        totalBookings,
        averageAttendance,
        totalRevenue,
        ...
        topPerformingClasses: classes.slice(0, 5).map((c) => ({
          ...
          revenue:
            (c.price || 0) * bookings.filter((b) => b.class_id === c.id).length,
        })),
        underperformingClasses: [],
        predictionStats: {
            ...
            {insights.smartInsights.map((insight, index) => (
              ...
              >
                <div className="flex items-start gap-3">
                  {insight.type === "success" && (
                    <FiCheckCircle className="text-green-600 dark:text-green-300 mt-1" />
                  )}
                  {insight.type === "warning" && (
                    <FiAlertTriangle className="text-yellow-600 dark:text-yellow-300 mt-1" />
                  )}
                  {insight.type === "info" && (
                    <FiZap className="text-blue-600 dark:text-blue-300 mt-1" />
                  )}
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                {insights.totalClasses}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-green-600 dark:text-green-300">
              ↗ {insights.activeClasses} active
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-blue-600 dark:text-blue-300">
              {insights.averageAttendance}% attendance
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-600 dark:text-gray-300">
              {insights.weeklyClasses} this week
            </span>
          </div>
        </motion.div>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-950 rounded-lg transition-colors">
              <div>
                ...
                <p className="text-sm text-blue-600 dark:text-blue-300">
                  Predicted based on trends
                </p>
              </div>
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-300">
                {insights.predictionStats.nextWeekBookings}
              </p>
            </div>
            <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950 rounded-lg transition-colors">
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-300">
                ${insights.predictionStats.revenueProjection}
              </p>
            </div>
          </div>
        </div>
