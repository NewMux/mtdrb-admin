...
const tagsList = ['Beginner-friendly', 'High-Intensity', 'Mobility', 'Core', 'Cardio'];
...
const ClassFormDrawer: React.FC<ClassFormDrawerProps> = ({ isOpen, mode, classData, onClose, onSaved }) => {
  const [selectedTags, setSelectedTags] = useState<string[]>(classData?.tags || []);
  ...
  const [roomConflictWarning, setRoomConflictWarning] = useState<string | null>(null);
    ...
    const checkConflict = async () => {
      ...
      try {
        ...
        const { data: conflicts, error } = await supabase
          ...
          .gte('starttime', new Date(startTime.getTime() - 2 * 60 * 60 * 1000).toISOString()) // look back 2h for safety
          .lte('starttime', new Date(endTime.getTime() + 2 * 60 * 60 * 1000).toISOString()); // look ahead 2h for safety
    ...
    const checkRoomConflict = async () => {
      ...
      try {
        ...
        const { data: conflicts, error } = await supabase
          ...
          .eq('room', room)
          .neq('id', classData?.id || '')
          .gte('starttime', new Date(startTime.getTime() - 2 * 60 * 60 * 1000).toISOString())
          .lte('starttime', new Date(endTime.getTime() + 2 * 60 * 60 * 1000).toISOString());
        if (error) throw error;
        if (conflicts && conflicts.length > 0) {
          for (const c of conflicts) {
            const cStart = new Date(c.starttime);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (mode === 'add') {
        ...
        result = await supabase.from('classes').update(payload).eq('id', classData.id);
      }
      if (result?.error) throw result.error;
      toast.success(`Class ${mode === 'add' ? 'created' : 'updated'} successfully!`);
      if (onSaved) onSaved();
      onClose();
    } catch (err: any) {
        ...
        <SmartModal
          ...
          footer={
            <div className="flex items-center justify-end space-x-3">
              ...
              <SmartButton variant="primary" onClick={handleSubmit} loading={submitLoading}>
                {submitLoading ? 'Saving...' : (mode === 'add' ? 'Create Class' : 'Update Class')}
              </SmartButton>
            </div>
          }
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {classTypes.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </AppleSelect>
                ...
                <div className="space-y-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  {watch('repeat_pattern') === 'custom' && (
                    <div className="p-4 bg-blue-50 rounded-xl border border-blue-200 mt-2">
                      <div className="text-blue-900 font-semibold mb-2">Custom Repeat Options</div>
                      <div className="text-blue-700 text-sm mb-2">(Coming soon: select specific days, intervals, and more!)</div>
            ...
            {/* Location & Staff */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Location & Staff</h3>
