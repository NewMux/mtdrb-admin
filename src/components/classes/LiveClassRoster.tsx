...
export const LiveClassRoster: React.FC<LiveClassRosterProps> = ({
  ...
  const fetchRoster = useCallback(async () => {
    try {
      ...
      const formattedRoster: ClassMember[] =
        data?.map((booking) => ({
          id: booking.id,
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-3">
            <div
              className={`w-3 h-3 rounded-full ${getClassStatusColor(classDetails?.status || "scheduled")}`}
            ></div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span>
              {stats.total}/{classDetails?.capacity}
            </span>
          </div>
          <div className="flex items-center space-x-2">
          ...
          {roster.length === 0 ? (
            ...
            roster.map((member) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`p-2 rounded-lg ${getStatusColor(member.status)}`}
                  >
