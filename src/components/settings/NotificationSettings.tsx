...
import { FiBell, FiMail, FiPhone, FiSettings, FiClock, FiUsers, FiZap, FiSave } from "react-icons/fi";

interface NotificationSettingsProps {
  refreshKey: number;
}

export const NotificationSettings: React.FC<NotificationSettingsProps> = ({ refreshKey }) => {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {channels.map((channel) => (
            <div key={channel.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
            <div>
              ...
              <p className="text-sm text-gray-600">Limit notifications during specified hours</p>
            </div>
            <button
              onClick={() => handleToggle('quietHours.enabled', !settings.quietHours.enabled)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('quietHours.start', e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('quietHours.end', e.target.value)}
                  className="w-full px-4 py-3 bg-gray-50 rounded-2xl border border-gray-200 focus:border-blue-600 focus:ring-0"
                />
              </div>
            </div>
        ...
        <div className="space-y-3">
          {notificationTypes.map((type) => (
            <div key={type.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
              ...
              <button
                onClick={() => handleToggle(`preferences.${type.key}`, !type.enabled)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900">Frequency Settings</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Marketing Notifications</label>
            <select
              value={settings.frequency.marketing}
              onChange={(e) => handleInputChange('frequency.marketing', e.target.value)}
              className="w-full px-4 py-3 bg-gray-50 rounded-2xl border border-gray-200 focus:border-blue-600 focus:ring-0"
            >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">System Updates</label>
            <select
              value={settings.frequency.updates}
              onChange={(e) => handleInputChange('frequency.updates', e.target.value)}
              className="w-full px-4 py-3 bg-gray-50 rounded-2xl border border-gray-200 focus:border-blue-600 focus:ring-0"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleInputChange('frequency.alerts', e.target.value)}
              className="w-full px-4 py-3 bg-gray-50 rounded-2xl border border-gray-200 focus:border-blue-600 focus:ring-0"
            >
              <option value="immediate">Immediate</option>
      ...
      {/* Test Notifications */}
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-900 mb-6">Test Notifications</h2>
