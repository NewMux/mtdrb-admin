...
import { FiServer, FiGlobe, FiClock, FiMapPin, FiDatabase, FiWifi, FiSave } from "react-icons/fi";

interface SystemSettingsProps {
  refreshKey: number;
}

export const SystemSettings: React.FC<SystemSettingsProps> = ({ refreshKey }) => {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Configure core system settings and operational parameters</p>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900">Performance & Limits</h2>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="space-y-4">
          {[
            { key: 'maintenanceMode', label: 'Maintenance Mode', description: 'Put system in maintenance mode for updates' },
            { key: 'autoUpdates', label: 'Automatic Updates', description: 'Enable automatic system updates' },
            { key: 'debugMode', label: 'Debug Mode', description: 'Enable debug logging and error reporting' },
            { key: 'enableLogging', label: 'System Logging', description: 'Log system events and user activities' }
          ].map((toggle) => (
            <div key={toggle.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
              <div>
                <h3 className="font-medium text-gray-900">{toggle.label}</h3>
                <p className="text-sm text-gray-600">{toggle.description}</p>
              </div>
              <button
                onClick={() => handleInputChange(toggle.key, !settings[toggle.key as keyof typeof settings])}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  settings[toggle.key as keyof typeof settings] ? 'bg-blue-600' : 'bg-gray-300'
                }`}
              >
                <span
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900">Backup & Maintenance</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
