...
import { FiTarget, FiUsers, FiTrendingUp, FiAlertTriangle, FiZap, FiGift, FiClock, FiAward } from 'react-icons/fi';
import { motion } from 'framer-motion';
import { getSmartPromotionInsights, SmartInsight } from '../../api/promotionInsights';
    ...
    return (
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="animate-pulse bg-gray-100 h-32 rounded-2xl"></div>
          ))}
        </div>
      </div>
    );
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900">AI Campaign Insights</h2>
            <p className="text-sm text-gray-600">Smart recommendations to boost your campaigns</p>
          </div>
        </div>
        ...
        {insights.map((insight, index) => (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-start space-x-3">
                <div className={`p-2 rounded-lg ${
                  insight.category === 'churn' ? 'bg-red-100' :
                  insight.category === 'acquisition' ? 'bg-blue-100' :
                  insight.category === 'engagement' ? 'bg-green-100' :
                  'bg-purple-100'
                }`}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mb-2">{insight.detectedIssue}</p>
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-700 font-medium">{insight.recommendedAction}</p>
      ...
      <div className="mt-6 pt-4 border-t border-gray-100">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>💡 AI analyzed {insights.reduce((sum, insight) => sum + insight.dataPoints, 0).toLocaleString()} data points to generate these insights</span>
