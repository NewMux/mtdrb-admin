...
const SmartPromotionAnalytics: React.FC<SmartPromotionAnalyticsProps> = ({ refreshKey }) => {
  const analytics = {
    conversionRates: [
      { period: "This Week", rate: 24.5, conversions: 89, revenue: "$4,450" },
      { period: "Last Week", rate: 21.3, conversions: 67, revenue: "$3,350" },
      { period: "This Month", rate: 23.1, conversions: 234, revenue: "$11,700" },
    ],
    channels: [
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Smart Promotion Analytics</h1>
          <p className="text-gray-600 mt-1">Advanced performance insights and ROI analysis</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {analytics.conversionRates.map((metric, index) => (
          <div key={index} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-3xl font-bold text-gray-900 mb-2">{metric.rate}%</p>
            <p className="text-sm text-gray-600 mb-3">{metric.conversions} conversions</p>
            <p className="text-lg font-semibold text-green-600">{metric.revenue}</p>
          </div>
        ))}
      </div>
            ...
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold text-purple-600">{channel.rate}%</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-4">ROI by Campaign</h3>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            {[
              { icon: FiTrendingUp, title: "Conversion Trending Up", desc: "24.5% this week vs 21.3% last week" },
              { icon: FiUsers, title: "In-App Performs Best", desc: "23.3% conversion rate, lowest cost" },
              { icon: FiBarChart, title: "Peak Performance", desc: "Tuesday 2-4 PM shows highest engagement" },
            ].map((insight, index) => (
              <div key={index} className="flex items-start space-x-3 p-3 bg-purple-50 rounded-2xl">
