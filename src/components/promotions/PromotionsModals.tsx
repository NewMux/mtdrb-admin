...
import { 
  FiX, FiGift, FiUsers, FiCalendar, FiTarget, FiZap, FiCpu, 
  FiCopy, FiEye, FiPlay, FiSettings, FiTrendingUp, FiAward,
  FiMail, FiPhone, FiSmartphone, FiShare2, FiCheck
} from 'react-icons/fi';
...
export const CampaignBuilderModal: React.FC<CampaignBuilderModalProps> = ({ 
  ...
  const channelOptions = [
    ...
    { value: "push", label: "Push Notifications", icon: <FiSmartphone className="h-4 w-4" />, reach: "70%" },
    { value: "in-app", label: "In-App", icon: <FiTarget className="h-4 w-4" />, reach: "60%" },
    { value: "social", label: "Social Media", icon: <FiShare2 className="h-4 w-4" />, reach: "40%" }
  ];
  ...
  const generateAICopy = async () => {
    ...
    setTimeout(() => {
      setFormData(prev => ({
        ...prev,
        aiCopy: "🎉 Exclusive offer just for you! Don't miss out on this amazing deal that's tailored specifically for our valued members. Limited time only!"
      }));
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              <div className="space-y-6">
                ...
                {/* AI Copy Generator */}
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">AI Copy Generation</h3>
                  <div className="bg-purple-50 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-2">
                        <FiCpu className="h-5 w-5 text-purple-600" />
                        <span className="font-medium text-gray-900">AI Copy Suggestion</span>
                      </div>
                      <button
                ...
                <div>
                  <label className="flex items-center space-x-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">Enable automation after launch</span>
                    </div>
                  </label>
                </div>
              </div>
...
export const WorkflowModal: React.FC<WorkflowModalProps> = ({ isOpen, onClose }) => {
  ...
  const actionOptions = [
    { value: "send-email", label: "Send email", icon: <FiMail className="h-4 w-4" /> },
    
    { value: "send-push", label: "Send push notification", icon: <FiSmartphone className="h-4 w-4" /> },
    { value: "apply-discount", label: "Apply discount", icon: <FiGift className="h-4 w-4" /> },
    { value: "schedule-followup", label: "Schedule follow-up", icon: <FiCalendar className="h-4 w-4" /> }
  ];
      ...
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900">Create Automation Workflow</h2>
              <button
                onClick={onClose}
                className="p-2 hover:bg-gray-100 rounded-xl transition-colors"
              >
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {actionOptions.map(action => (
                        <label key={action.value} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer">
                          <input
                            type="checkbox"
                  ...
                  <div>
                    <label className="flex items-center space-x-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer">
                      ...
                      <div className="flex items-center space-x-2">
                        <FiZap className="h-5 w-5 text-purple-600" />
                        <span className="font-medium">Activate workflow immediately</span>
                      </div>
                    </label>
                  </div>
                </div>
                    ...
                    <div className="space-y-3">
                      <div className="bg-white p-3 rounded-xl">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <span className="text-sm font-medium text-gray-900">High Impact</span>
                        </div>
                        <p className="text-xs text-gray-600">Birthday workflows show 45% higher engagement</p>
                      </div>
                      <div className="bg-white p-3 rounded-xl">
                        <div className="flex items-center space-x-2 mb-1">
                          <FiAward className="h-4 w-4 text-blue-600" />
                          <span className="text-sm font-medium text-gray-900">Best Practice</span>
                        </div>
                        <p className="text-xs text-gray-600">Combine email + push for 23% better conversion</p>
                      </div>
                      <div className="bg-white p-3 rounded-xl">
                        <div className="flex items-center space-x-2 mb-1">
                        ...
                        <p className="text-xs text-gray-600">Add follow-up action for inactive triggers</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">
                        {campaign.discountValue}{campaign.discountType === 'percentage' ? '%' : '$'}
                      </div>
                      <div className="text-sm text-gray-600">Discount</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{campaign.channels.length}</div>
                ...
                {/* AI Copy Preview */}
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">AI Copy Preview</h3>
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="font-medium text-gray-900 capitalize">{campaign.discountType}</div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="font-medium text-gray-900">{campaign.startDate}</div>
                      </div>
                      <div>
                        <span className="text-sm text-gray-600">End Date</span>
                        <div className="font-medium text-gray-900">{campaign.endDate}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Channels */}
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Distribution Channels</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {campaign.channels.map((channel, index) => (
                      <div key={index} className="bg-white border border-gray-200 rounded-xl p-3">
                        <div className="flex items-center space-x-2">
                          {channel === 'email' && <FiMail className="h-4 w-4 text-blue-600" />}
          
                          {channel === 'push' && <FiSmartphone className="h-4 w-4 text-purple-600" />}
                          {channel === 'in-app' && <FiTarget className="h-4 w-4 text-orange-600" />}
                          {channel === 'social' && <FiShare2 className="h-4 w-4 text-pink-600" />}
                ...
                <div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-4">
                    ...
                    <p className="text-sm text-gray-600">180 members will receive this campaign</p>
                  </div>
                </div>
              </div>
            </div>
