...
export const QuickNavigation: React.FC<QuickNavProps> = ({
  ...
  const current =
    navigationConfig[currentPage as keyof typeof navigationConfig];
    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {connections.map((connection) => (
          ...
          >
            <div className="text-blue-600">{connection.icon}</div>
            <div>
              <p className="text-sm font-medium text-blue-900">
                {connection.title}
              </p>
              <p className="text-xs text-blue-600">
                {connection.description.split(" ").slice(0, 2).join(" ")}
              </p>
            </div>
          </motion.button>
        ))}
...
// Create a default export that includes all navigation components
