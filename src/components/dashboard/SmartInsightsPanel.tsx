      ...
      {insights.map((insight, idx) => (
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className={`p-2 rounded-xl ${colorBg[insight.color]} dark:bg-opacity-80`}>{insight.icon}</div>
            <span className="text-sm font-medium opacity-80">{insight.title}</span>
          </div>
          <div className="text-2xl font-bold">{insight.value}</div>
          <div className="text-sm opacity-70">{insight.description}</div>
          {insight.context && <div className="text-xs mt-1 opacity-70">{insight.context}</div>}
        </div>
      ))}
    </div>
  </section>
