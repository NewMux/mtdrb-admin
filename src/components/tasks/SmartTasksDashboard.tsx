...
import { FiTrendingUp, FiTrendingDown, FiCheckSquare, FiClock, FiTarget, FiUsers } from 'react-icons/fi';
...
const TaskInsight: React.FC<TaskInsightProps> = ({ icon, title, description, action, priority }) => {
...
export const SmartTasksDashboard: React.FC<SmartTasksDashboardProps> = ({ refreshKey, stats }) => {
  const kpis = [
    {
      title: 'Active Tasks',
      value: 23,
  ...
  const insights = [
    ...
    },
    {
      ...
      description: '3 classes tomorrow need equipment setup and material preparation',
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Smart Tasks Dashboard</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">AI-powered task management and productivity insights</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button className="p-4 bg-blue-50 dark:bg-blue-900/30 rounded-2xl hover:bg-blue-100 dark:hover:bg-blue-800/50 transition-colors text-left">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Schedule Member Calls</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">Auto-assign follow-up calls</p>
          </button>
          <button className="p-4 bg-green-50 dark:bg-green-900/30 rounded-2xl hover:bg-green-100 dark:hover:bg-green-800/50 transition-colors text-left">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-gray-100">Bulk Task Creation</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">Create recurring tasks</p>
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-3">
            {[
              { name: 'Member Management', count: 12, color: 'bg-blue-500', percentage: 52 },
              { name: 'Class Preparation', count: 6, color: 'bg-green-500', percentage: 26 },
              { name: 'Staff Management', count: 3, color: 'bg-yellow-500', percentage: 13 },
              { name: 'Facility Maintenance', count: 2, color: 'bg-purple-500', percentage: 9 }
            ].map((category) => (
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            {[
              { name: 'Sarah Johnson', role: 'Front Desk', completion: 95, tasks: 8 },
              { name: 'Mike Chen', role: 'Trainer', completion: 89, tasks: 12 },
              { name: 'Emma Davis', role: 'Manager', completion: 92, tasks: 15 },
              { name: 'Alex Rodriguez', role: 'Maintenance', completion: 87, tasks: 6 }
            ].map((member) => (
