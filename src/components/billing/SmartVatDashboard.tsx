...
export default function SmartVatDashboard({ tenantId, refreshKey }: SmartVatDashboardProps) {
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'returns' | 'compliance' | 'analytics'>('overview');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Smart VAT Dashboard</h2>
          <p className="text-gray-600">Comprehensive VAT management and compliance monitoring</p>
        </div>
      ...
      {selectedTab === 'overview' && (
        <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <span className={`text-sm ${dashboardData.vatGrowthPercentage > 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center">
                    <p className={`text-2xl font-bold ${getComplianceColor(dashboardData.complianceScore)}`}>
                      {dashboardData.complianceScore}%
                    </p>
                    <span className="ml-2">{getComplianceIcon(dashboardData.complianceScore)}</span>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Recent VAT Transactions</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">VAT Amount</th>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">VAT Returns</h3>
            ...
            {((vatReturns || []).length === 0) ? (
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No VAT returns</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Generate your first VAT return to get started.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Net VAT</th>
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {(vatReturns || []).map((vatReturn) => (
                      <tr key={vatReturn.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                          {vatReturn.return_period_start} - {vatReturn.return_period_end}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                          <div className="flex items-center">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                          {vatReturn.submitted_at ? new Date(vatReturn.submitted_at).toLocaleDateString() : 'N/A'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            vatReturn.status === 'draft' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200' :
                            vatReturn.status === 'submitted' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200' :
                            vatReturn.status === 'accepted' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
                            'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'
                          }`}>
                            {vatReturn.status}
                          </span>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {vatReturn.status === 'draft' && (
                              <SmartButton
                                ...
                                variant="primary"
                                icon={<FiSend size={16} />}
                                onClick={() => handleSubmitVatReturn(vatReturn.id)}
                                title="Submit"
                              />
                            )}
                          </div>
      ...
      {selectedTab === 'compliance' && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Compliance Overview</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <div className={`text-3xl font-bold ${getComplianceColor(dashboardData?.complianceScore ?? 0)}`}>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Compliance Checklist</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex items-center">
                  <FiCalendar className="mr-3 text-gray-400 dark:text-gray-500" />
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">VAT Returns Filed on Time</span>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  (dashboardData?.overdueReturns ?? 0) === 0 ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'
                }`}>
                  {(dashboardData?.overdueReturns ?? 0) === 0 ? 'Compliant' : `${dashboardData?.overdueReturns ?? 0} Overdue`}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">VAT Payments Made on Time</span>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  (dashboardData?.overduePayments ?? 0) === 0 ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200'
                }`}>
                  {(dashboardData?.overduePayments ?? 0) === 0 ? 'Compliant' : `${dashboardData?.overduePayments ?? 0} Overdue`}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  (dashboardData?.upcomingDeadlines ?? 0) === 0 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {(dashboardData?.upcomingDeadlines ?? 0) === 0 ? 'No Deadlines' : `${dashboardData?.upcomingDeadlines ?? 0} Due Soon`}
                </span>
              </div>
            </div>
          </div>
      ...
      {selectedTab === 'analytics' && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">VAT Analytics</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-sm font-medium text-gray-600 mb-3">VAT by Country</h4>
                <div className="space-y-3">
                  {(dashboardData?.vatByCountry || []).map((country, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <span className="text-sm text-gray-900">{country.country_name}</span>
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-600 mb-3">Period Comparison</h4>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-900">Current Period</span>
                    <span className="text-sm font-medium text-green-600">
                      {formatCurrency(dashboardData?.currentPeriodVat ?? 0)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-900">Previous Period</span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-medium ${
                      (dashboardData?.vatGrowthPercentage ?? 0) > 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {(dashboardData?.vatGrowthPercentage ?? 0) > 0 ? '+' : ''}{(dashboardData?.vatGrowthPercentage ?? 0).toFixed(1)}%
                    </span>
                  </div>
                </div>
              </div>
