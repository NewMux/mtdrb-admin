...
import { 
  FiDollarSign, FiCalendar, FiTag, FiFileText, FiUpload, FiX, 
  FiCheck, FiAlertCircle, FiInfo, FiSearch, FiClock, FiTrendingUp,
  FiShield, FiBarChart2, FiZap, FiStar, FiTarget, FiActivity,
  FiHeart, FiSave, FiArrowRight, FiArrowLeft, FiRefreshCw
} from 'react-icons/fi';
...
const SMART_CATEGORIZATION = {
  ...
  'OFFICE': { category: 'Office Supplies', description: 'Office Supplies Purchase' },
  'SUBSCRIPTION': { category: 'Subscriptions', description: 'Service Subscription' },
...
export function AddExpenseModal({ 
  ...
  const findSimilarExpenses = async () => {
    ...
    try {
      const { data } = await supabase
        .from('expenses')
        .select('*')
        .or(`title.ilike.%${watchedTitle}%,description.ilike.%${watchedDescription}%`)
        .order('created_at', { ascending: false })
        .limit(5);

      if (data) {
        const similar = data.map(expense => ({
          id: expense.id,
          title: expense.title,
  ...
  const Footer = () => (
    <div className="sticky bottom-0 left-0 w-full bg-white border-t border-gray-200 py-4 px-8 flex items-center justify-between z-10">
      <AppleButton variant="secondary" onClick={onClose}>Cancel</AppleButton>
      <AppleButton variant="primary" onClick={handleSubmit(onSubmit)} loading={isSubmitting} disabled={!isValid || isSubmitting}>
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8" onKeyDown={handleKeyDown}>
            ...
            <div className="card p-6 space-y-6">
              <h3 className="text-lg font-semibold flex items-center mb-4"><FiDollarSign className="mr-2" />Expense Details</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AppleInput label="Title" {...register('title')} error={errors.title?.message} required placeholder="Enter expense title" />
                <AppleInput label="Amount" type="number" step="0.01" {...register('amount')} error={errors.amount?.message} required placeholder="0.00" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AppleInput label="Date" type="date" {...register('date')} error={errors.date?.message} required />
                <AppleSelect label="Category" {...register('category')} error={errors.category?.message} required>
                  <option value="">Select Category</option>
                  {EXPENSE_CATEGORIES.map(cat => (<option key={cat} value={cat}>{cat}</option>))}
                </AppleSelect>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AppleInput label="Vendor" {...register('vendor')} error={errors.vendor?.message} placeholder="Enter vendor name" onChange={handleVendorInput} ref={vendorInputRef} />
                <AppleSelect label="Payment Method" {...register('payment_method')} error={errors.payment_method?.message} required>
                  <option value="">Select Payment Method</option>
                  {PAYMENT_METHODS.map(method => (<option key={method} value={method}>{method.charAt(0).toUpperCase() + method.slice(1).replace('_', ' ')}</option>))}
                </AppleSelect>
              </div>
              <AppleTextarea label="Description" {...register('description')} error={errors.description?.message} rows={3} placeholder="Describe the expense..." />
            </div>

            {/* VAT & Receipt Section */}
            <div className="card p-6 space-y-6">
              <h3 className="text-lg font-semibold flex items-center mb-4"><FiBarChart2 className="mr-2" />VAT & Receipt</h3>
              <div className="flex items-center space-x-4">
                <AppleToggle label="VAT Included" checked={watchedVatIncluded} onChange={(checked) => setValue('vat_included', checked)} />
                {watchedVatIncluded && (
                  <AppleSelect label="VAT Rate" {...register('vat_rate')} error={errors.vat_rate?.message}>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                <div className="flex justify-between text-sm"><span>Subtotal:</span><span>${totalAmount.toFixed(2)}</span></div>
                {watchedVatIncluded && (<div className="flex justify-between text-sm"><span>VAT ({watch('vat_rate')}%):</span><span>${vatAmount.toFixed(2)}</span></div>)}
                <div className="flex justify-between font-medium border-t pt-2"><span>Total:</span><span>${finalAmount.toFixed(2)}</span></div>
              </div>
              <div className="space-y-4">
                <h4 className="font-medium flex items-center"><FiUpload className="mr-2" />Receipt Upload</h4>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors" onDrop={handleDrop} onDragOver={handleDragOver}>
                  {selectedFile ? (
                    <div className="space-y-4">
                      {filePreview ? (<div className="flex justify-center"><img src={filePreview} alt="Receipt preview" className="max-w-xs max-h-32 object-contain rounded-lg" /></div>) : (<div className="flex justify-center"><FiFileText className="h-12 w-12 text-green-500" /></div>)}
                      ...
                      <button type="button" onClick={() => { setSelectedFile(null); setFilePreview(null); setValue('receipt_file', null); }} className="text-sm text-red-600 hover:text-red-700">Remove file</button>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <div className="flex justify-center"><FiUpload className="h-12 w-12 text-gray-400" /></div>
                      <div><p className="text-sm text-gray-600">Drag and drop your receipt here, or <button type="button" onClick={() => fileInputRef.current?.click()} className="text-blue-600 hover:text-blue-700 font-medium">browse</button></p><p className="text-xs text-gray-500 mt-1">Supports: JPG, PNG, GIF, PDF (max 10MB)</p></div>
            ...
            <div className="card p-6 space-y-6">
              <h3 className="text-lg font-semibold flex items-center mb-4"><FiShield className="mr-2" />Settings & Notes</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AppleSelect label="Status" {...register('status')} error={errors.status?.message} required>
                  <option value="">Select Status</option>
                  {EXPENSE_STATUSES.map(status => (<option key={status} value={status}>{status.charAt(0).toUpperCase() + status.slice(1)}</option>))}
                </AppleSelect>
                <div className="flex items-center space-x-3"><AppleToggle label="Recurring Expense" checked={watchedRecurring} onChange={(checked) => setValue('recurring', checked)} /></div>
              </div>
              {watchedRecurring && (<AppleSelect label="Recurring Frequency" {...register('recurring_frequency')} error={errors.recurring_frequency?.message}><option value="">Select Frequency</option>{RECURRING_FREQUENCIES.map(freq => (<option key={freq} value={freq}>{freq.charAt(0).toUpperCase() + freq.slice(1)}</option>))}</AppleSelect>)}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AppleTextarea label="Internal Notes" {...register('internal_notes')} error={errors.internal_notes?.message} rows={3} placeholder="Private notes for staff only..." />
                <AppleTextarea label="Public Notes" {...register('public_notes')} error={errors.public_notes?.message} rows={3} placeholder="Notes visible to members..." />
              </div>
            </div>
