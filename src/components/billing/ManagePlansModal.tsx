...
import { FiDollarSign, FiCalendar, FiUsers, FiCheck, FiX, FiEdit2, FiTrash2, FiAlertTriangle, FiPlus, FiEdit } from 'react-icons/fi';
...
export function DeleteConfirmationModal({ isOpen, onClose, onConfirm, planName }: DeleteConfirmationModalProps) {
  return (
    <SmartModal
      isOpen={isOpen}
      onClose={onClose}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.name || formData.price <= 0) {
        throw new Error('Please fill in all required fields and ensure price is greater than 0');
      }
      ...
      if (editingPlan) {
        ...
        if (formData.price !== editingPlan.price) {
          ...
          if (subscriptions && subscriptions.length > 0) {
            throw new Error('Cannot modify price of a plan that has active subscriptions');
          }
        }
              ...
              <div className="space-y-4">
                ...
                {loading ? (
                  ...
                  <div className="grid gap-4">
                    {plans.map((plan) => (
                      <div key={plan.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-xl hover:bg-gray-50">
                        <div className="flex-1">
                          <h4 className="font-medium text-gray-900">{plan.name}</h4>
                          <p className="text-sm text-gray-500">{plan.description}</p>
                          <div className="flex items-center space-x-4 mt-2">
                            <span className="text-sm font-medium text-blue-600">{plan.price} BHD</span>
