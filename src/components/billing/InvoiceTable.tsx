...
export default function InvoiceTable() {
  ...
  const paginated = MOCK_INVOICES.slice(
      ...
      <table className="w-full min-w-[900px] table-auto">
        <thead>
          <tr className="border-b border-blue-100 dark:border-gray-700 bg-blue-50 dark:bg-gray-700">
            <th className="py-3 px-4 text-left text-xs font-semibold text-blue-700 dark:text-blue-300 max-w-[140px] whitespace-nowrap">
              Invoice ID
            </th>
            <th className="py-3 px-4 text-left text-xs font-semibold text-blue-700 dark:text-blue-300 w-48 whitespace-nowrap">
              Member
            </th>
            <th className="py-3 px-4 text-left text-xs font-semibold text-blue-700 dark:text-blue-300 w-24 whitespace-nowrap">
              Type
            </th>
            <th className="py-3 px-4 text-left text-xs font-semibold text-blue-700 dark:text-blue-300 w-32 whitespace-nowrap">
              Amount
            </th>
            <th className="py-3 px-4 text-left text-xs font-semibold text-blue-700 dark:text-blue-300 w-24 whitespace-nowrap">
              Status
        ...
        <tbody>
          {paginated.map((inv, idx) => {
            ...
            return (
              <tr
                key={inv.id}
                className={`border-b border-blue-50 dark:border-gray-700 ${inv.status === "Overdue" ? "bg-red-50/40 dark:bg-red-900/20" : "bg-white dark:bg-gray-800"} transition`}
              >
                {/* Invoice ID */}
                ...
                <td className="py-3 px-4 flex items-center gap-3 w-48 whitespace-nowrap">
                  ...
                  <div>
                    <a
                      ...
                      className="font-semibold text-blue-900 dark:text-blue-100 hover:underline whitespace-nowrap"
                    >
                      {inv.member.name}
                    </a>
                ...
                {/* Type */}
                <td className="py-3 px-4 text-blue-700 dark:text-blue-300 font-medium w-24 whitespace-nowrap">
                  {inv.type}
                </td>
                {/* Amount */}
                ...
                <td className="py-3 px-4 w-24 whitespace-nowrap">
                  ...
                  >
                    ...
                    {inv.status === "Overdue" && (
                      <FiAlertCircle className="text-red-500 dark:text-red-400 animate-pulse" />
                    )}
                    {inv.status}
                  </span>
                </td>
