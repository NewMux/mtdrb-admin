...
export default function EditBranchModal({
  ...
  const [formData, setFormData] = React.useState<BranchFormData>({
    ...
    business_hours: branch.business_hours || { ...defaultBusinessHours },
    booking_policies: branch.booking_policies || { ...defaultBookingPolicies },
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await api.branches.update(branch.id, updateData);

      if (error) throw error;

      toast.success(t("branches.branchUpdatedSuccessfully") || "Branch updated successfully!");
      setLoading(false);
      onSuccess?.();
      onClose();
    } catch (error: any) {
  ...
  const footer = (
    <>
      ...
      >
        {loading ? (t("branches.updating") || "Updating...") : (t("branches.updateBranch") || "Update Branch")}
      </SmartButton>
    </>
  );
  ...
  return (
    <UnifiedModal
      ...
      subtitle={t("branches.editBranchDesc") || "Update branch information"}
      footer={footer}
      maxWidth="4xl"
      slideFrom="right"
    >
                ...
                >
                  ...
                  <div className={`flex items-center ${isRTL ? 'flex-row-reverse space-x-reverse' : ''} space-x-3`}>
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label htmlFor={`edit-${day}-open`} className="text-sm text-gray-600 dark:text-gray-400 min-w-[50px]">
