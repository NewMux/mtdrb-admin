...
import { 
  ...
  FiX,
  FiChevronDown,
  FiDollarSign, FiTrendingUp, FiRefreshCw, FiDownload
} from 'react-icons/fi';
  ...
  const displayValue = () => {
    if (multiSelect) {
      ...
      if (selectedArray.length === 1) {
        return options.find(opt => opt.value === selectedArray[0])?.label || label;
      }
            ...
            {options.map((option) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-md text-sm hover:bg-gray-100 transition-colors ${
                  (multiSelect && Array.isArray(selected) && selected.includes(option.value)) ||
            ...
            <div className="border-t pt-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input
                  ...
                  onChange={(e) => handleDateSelect(new Date(e.target.value), false)}
                  className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
            </div>
...
export default function AnalyticsFilterBar({ filters, onFiltersChange, onClearAll }: AnalyticsFilterBarProps) {
  ...
  return (
    <div className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
      <div className="p-4">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3 transition-all duration-300 ${
          ...
          <TagSelector
            tags={['VIP', 'New Member', 'Premium', 'Student', 'Corporate', 'Family']}
