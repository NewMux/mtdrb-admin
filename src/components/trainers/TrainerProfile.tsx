...
import { FiEdit2, FiStar, FiCalendar, FiDollarSign, FiMessageSquare, FiFileText, FiUsers } from 'react-icons/fi';
...
export default function TrainerProfile({ trainerId, onClose }: Props) {
  ...
  const [earnings, setEarnings] = useState<any>({ thisMonth: 0, lastMonth: 0 });
  ...
  const processPerformanceData = (data: any[]): Performance[] => {
  ...
  const calculateSessionRevenue = (session: any): number => {
    if (!trainer) return 0;

    let revenue = 0;
    const duration = (new Date(session.end_time).getTime() - new Date(session.start_time).getTime()) / (1000 * 60 * 60); // hours

    if (trainer.salary_type === 'hourly' || trainer.salary_type === 'hybrid') {
      revenue += trainer.hourly_rate * duration;
    }

    if (trainer.salary_type === 'commission' || trainer.salary_type === 'hybrid') {
    ...
    try {
      ...
      if (!error && payments) {
        ...
        const lastMonthEarnings = payments
          .filter(p => new Date(p.payment_date) >= lastMonth && new Date(p.payment_date) < thisMonthStart)
          .reduce((sum, p) => sum + p.amount, 0);

        setEarnings({ thisMonth: thisMonthEarnings, lastMonth: lastMonthEarnings });
      }
    } catch (error) {
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <img
              src={trainer.profile_image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(trainer.name)}&background=random`}
              alt={trainer.name}
              ...
              <div className="flex items-center mt-1">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                  ${trainer.status === 'active' ? 'bg-green-100 text-green-800' :
                    trainer.status === 'on_leave' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="flex items-center ml-4">
                  ...
                  <span className="ml-1 text-sm text-gray-600">{trainer.rating.toFixed(1)}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm font-medium text-gray-500">Total Sessions</span>
            </div>
            <p className="mt-2 text-2xl font-semibold text-gray-900">{trainer.total_sessions}</p>
          </div>
          
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center">
              <FiStar className="h-5 w-5 text-gray-400" />
              <span className="ml-2 text-sm font-medium text-gray-500">Active Members</span>
            </div>
            <p className="mt-2 text-2xl font-semibold text-gray-900">{trainer.active_members}</p>
          </div>
          
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center">
              <FiDollarSign className="h-5 w-5 text-gray-400" />
              <span className="ml-2 text-sm font-medium text-gray-500">Payment Type</span>
            </div>
            <p className="mt-2 text-lg font-semibold text-gray-900 capitalize">{trainer.salary_type}</p>
          </div>
          
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex items-center">
              <FiFileText className="h-5 w-5 text-gray-400" />
              <span className="ml-2 text-sm font-medium text-gray-500">Documents</span>
            </div>
            <p className="mt-2 text-2xl font-semibold text-gray-900">{documents.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        {activeTab === 'info' && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
                {Object.entries(trainer.social_links).map(([platform, url]) => (
                  url && (
                    <a
                      key={platform}
                      href={url}
          ...
          <div className="space-y-6">
            {feedback.map((item) => (
              <div key={item.id} className="border-b border-gray-200 pb-6">
                <div className="flex items-center">
                  <img
                    src={item.member.profile_image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(item.member.name)}&background=random`}
                ...
                >
                  ...
                  <div className="mt-3 space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{month.total_sessions}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Attendance Rate</span>
                      <span className="font-medium">{month.attendance_rate.toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="mt-2 text-2xl font-semibold text-gray-900">{trainer.total_sessions}</p>
            <p className="mt-1 text-sm text-green-600">+{recentClasses.length} this week</p>
          </div>
          
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500">Assigned Members</h3>
            <p className="mt-2 text-2xl font-semibold text-gray-900">{assignedMembers.length}</p>
            <p className="mt-1 text-sm text-blue-600">
              {assignedMembers.filter(m => m.status === 'Active').length} active
            </p>
          </div>
          
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500">This Month Earnings</h3>
            <p className="mt-2 text-2xl font-semibold text-gray-900">BHD {earnings.thisMonth.toFixed(2)}</p>
            <p className={`mt-1 text-sm ${earnings.thisMonth >= earnings.lastMonth ? 'text-green-600' : 'text-red-600'}`}>
              {earnings.thisMonth >= earnings.lastMonth ? '+' : ''}{((earnings.thisMonth - earnings.lastMonth) / Math.max(earnings.lastMonth, 1) * 100).toFixed(1)}%
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mt-1 flex items-center">
              ...
              <span className="ml-1 text-sm text-gray-600">{trainer.rating?.toFixed(1) || 'N/A'}</span>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            {assignedMembers.length > 0 ? (
                ...
                {assignedMembers.slice(0, 5).map((member) => (
                  <div key={member.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Joined {dayjs(member.joined_at).format('MMM DD, YYYY')}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 mt-1">{member.membership_type}</p>
                      <span className={`text-xs ${
                        member.payment_status === 'Paid' ? 'text-green-600' :
                        member.payment_status === 'Overdue' ? 'text-red-600' :
                        'text-yellow-600'
                      }`}>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">No assigned members</h3>
          ...
          <div className="p-6">
            {recentClasses.length > 0 ? (
                  ...
                  <div key={classItem.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {dayjs(classItem.start_time).format('MMM DD, YYYY • HH:mm')} - {dayjs(classItem.end_time).format('HH:mm')}
                      </p>
                      <p className="text-xs text-gray-500">{classItem.location}</p>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium text-gray-900">
                        {classItem.bookings?.[0]?.count || 0} / {classItem.capacity}
                      </div>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">No recent classes</h3>
                <p className="mt-1 text-sm text-gray-500">
