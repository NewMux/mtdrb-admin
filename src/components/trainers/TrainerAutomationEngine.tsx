...
import { FiZap, FiPlay, FiPause, FiSettings, FiTrendingUp, FiUsers, FiCalendar, FiMessageSquare, FiCheckCircle, FiClock, FiTarget, FiActivity } from 'react-icons/fi';
import toast from 'react-hot-toast';
import { mockTrainerAutomationWorkflows, mockAutomationMetrics } from '../../api/mockTrainerData';
...
export default function TrainerAutomationEngine({ refreshKey }: TrainerAutomationEngineProps) {
  const [workflows, setWorkflows] = useState<AutomationWorkflow[]>(mockTrainerAutomationWorkflows.map(w => ({
    ...w,
    icon: w.icon === 'FiCalendar' ? FiCalendar : 
          w.icon === 'FiTrendingUp' ? FiTrendingUp :
          w.icon === 'FiTarget' ? FiTarget :
          w.icon === 'FiMessageSquare' ? FiMessageSquare :
          w.icon === 'FiUsers' ? FiUsers : FiCalendar
  })));
  const [metrics, setMetrics] = useState<AutomationMetric[]>(mockAutomationMetrics.map(m => ({
    ...m,
    icon: m.icon === 'FiZap' ? FiZap :
          m.icon === 'FiCheckCircle' ? FiCheckCircle :
          m.icon === 'FiActivity' ? FiActivity :
          m.icon === 'FiTrendingUp' ? FiTrendingUp : FiZap
  })));
  const [loading, setLoading] = useState(false);
  const [selectedWorkflow, setSelectedWorkflow] = useState<string | null>(null);
  const [isConfigModalOpen, setIsConfigModalOpen] = useState(false);
  const [configuringWorkflow, setConfiguringWorkflow] = useState<AutomationWorkflow | null>(null);
      ...
      <div className="bg-gradient-to-br from-purple-600 to-pink-700 rounded-2xl shadow-xl p-6 text-white">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-purple-100">Smart workflows and automated trainer management</p>
          </div>
        </div>
          ...
          {metrics.map((metric, index) => (
            <div key={index} className="bg-white bg-opacity-10 rounded-xl p-4">
              ...
              <div className="text-2xl font-bold">
                {metric.label === 'Revenue Impact' ? `$${metric.value.toLocaleString()}` : metric.value}
              </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 uppercase tracking-wider">{workflow.type}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 text-xs">
                  ...
                  <div className="text-center">
                    <div className="font-semibold text-gray-900">{workflow.triggerCount}</div>
                    <div className="text-gray-500">Triggers</div>
                  </div>
                  <div className="text-center">
                    <div className="font-semibold text-gray-900">${workflow.revenue}</div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900">Top Performing Workflows</h4>
            {workflows
            ...
            {workflows
              ...
              .map((workflow, index) => (
                <div key={workflow.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">{workflow.name}</div>
                    <div className="text-sm text-gray-600">{workflow.triggerCount} triggers</div>
                  </div>
            ...
            {workflows
              ...
              .map((workflow, index) => (
                <div key={workflow.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">{workflow.name}</div>
                    <div className="text-sm text-gray-600">${workflow.revenue} generated</div>
                  </div>
