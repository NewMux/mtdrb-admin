...
import { FiUser, FiPhone, FiMail, FiCalendar, FiTrendingUp, FiTrendingDown, FiClock, FiAlertTriangle, FiCheckCircle, FiXCircle, FiEdit, FiEye, FiTrash2, FiMessageCircle, FiFileText, FiTarget, FiDollarSign, FiBarChart, FiActivity, FiSend, FiDownload, FiPrinter, FiShare2 } from 'react-icons/fi';
...
const MemberDetailModal: React.FC<MemberDetailModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'attendance' | 'billing' | 'documents'>('profile');
  ...
  const getAttendanceStats = (attendance: string[]) => {
    ...
    const thisWeek = attendance.filter(date => new Date(date) >= lastWeek).length;
    const thisMonth = attendance.filter(date => new Date(date) >= lastMonth).length;
  ...
  const renderProfileTab = () => (
    ...
    >
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-gray-900">{member.age} years • {member.gender}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-gray-900">{member.emergencyContact || 'Not provided'}</p>
          </div>
        </div>
      </div>
      ...
      {/* Status & Tags */}
      <div className="bg-gray-50 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Status & Classification</h3>
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-blue-900">{attendanceStats.thisWeek}</p>
        </div>
        ...
        <div className="bg-green-50 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-green-900">{attendanceStats.thisMonth}</p>
        </div>
        
        <div className="bg-purple-50 rounded-xl p-4">
          <div className="flex items-center space-x-2 mb-2">
            <FiTarget className="w-5 h-5 text-purple-600" />
            <span className="text-sm font-medium text-purple-900">Total Visits</span>
          </div>
          <p className="text-2xl font-bold text-purple-900">{attendanceStats.total}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <div className="space-y-3">
          {member.attendance.slice(-5).reverse().map((date, index) => (
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-gray-900">{member.paymentMethod || 'Not specified'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500">Auto Renewal</label>
            <p className="text-gray-900">{member.autoRenewal ? 'Enabled' : 'Disabled'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500">Plan End Date</label>
            <p className="text-gray-900">{new Date(member.planEnd).toLocaleDateString()}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-lg font-bold ${
              billingInfo.isExpired ? 'text-red-600' : 
              billingInfo.isExpiringSoon ? 'text-yellow-600' : 'text-green-600'
            }`}>
          ...
          {billingInfo.isExpired && (
            <div className="p-3 bg-red-50 rounded-lg border border-red-200">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-medium text-red-800">Membership has expired</span>
              </div>
            </div>
          )}
  ...
  const renderDocumentsTab = () => (
    ...
    >
      ...
      <div className="bg-gray-50 rounded-xl p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">Waiver Form</span>
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-sm ${member.formsSubmitted.includes('waiver') ? 'text-green-600' : 'text-red-600'}`}>
                {member.formsSubmitted.includes('waiver') ? 'Completed' : 'Missing'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-sm ${member.formsSubmitted.includes('id_document') ? 'text-green-600' : 'text-red-600'}`}>
                {member.formsSubmitted.includes('id_document') ? 'Completed' : 'Missing'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-sm ${member.formsSubmitted.includes('emergency_contact') ? 'text-green-600' : 'text-red-600'}`}>
                {member.formsSubmitted.includes('emergency_contact') ? 'Completed' : 'Missing'}
              </span>
            </div>
          </div>
        </div>
