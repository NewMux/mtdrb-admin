...
import { 
  FiX, FiUser, FiHeart, FiTarget, FiDollarSign, FiSettings, 
  FiMessageSquare, FiFileText, FiCheck, FiArrowRight, FiArrowLeft,
  FiMail, FiPhone, FiCalendar, FiMapPin, FiShield, FiZap, FiAlertCircle, FiCheckCircle
} from 'react-icons/fi';
...
const SmartField: React.FC<SmartFieldProps> = ({
  ...
  const handleChange = (newValue: any) => {
    ...
    if (name === 'fitness_level' && newValue === 'Beginner' && onDependencyChange) {
    ...
    if (name === 'medical_conditions' && newValue.length > 0 && onDependencyChange) {
  ...
  const renderField = () => {
    const baseClasses = `w-full px-4 py-3 border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
      error ? 'border-red-300 bg-red-50' : isFocused ? 'border-blue-300 bg-blue-50' : 'border-gray-300 bg-white'
    }`;
  ...
  return (
    <div className="space-y-6">
      <div className={`flex items-center space-x-3 pb-4 ${
        gradient ? 'bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3 rounded-lg' : ''
      }`}>
...
const SmartTabs: React.FC<SmartTabsProps> = ({ tabs, activeTab, onTabChange }) => {
  return (
...
const MemberModal: React.FC<MemberModalProps> = ({
  ...
  const tabs = [
    ...
    { id: 'health', label: 'Health & Fitness', icon: <FiHeart className="w-4 h-4" /> },
    { id: 'goals', label: 'Goals & Preferences', icon: <FiTarget className="w-4 h-4" /> },
    { id: 'billing', label: 'Billing & Membership', icon: <FiDollarSign className="w-4 h-4" /> },
    { id: 'access', label: 'Access & Features', icon: <FiSettings className="w-4 h-4" /> },
    { id: 'communication', label: 'Communication', icon: <FiMessageSquare className="w-4 h-4" /> },
  ...
  const requiredFieldsByTab: { [tab: string]: (keyof Member)[] } = {
    ...
    billing: ['membership_status', 'membership_type', 'billing_cycle', 'start_date', 'end_date'],
  ...
  const getTabStatus = (tab: string) => {
    ...
    return { hasError, isComplete: isComplete && fields.length > 0 && !hasError };
  };
  ...
  const handleArrayFieldChange = useCallback((field: keyof Member, value: string, checked: boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: checked 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {['Shoulder', 'Knee', 'Back', 'Ankle', 'Wrist', 'Hip', 'None'].map(injury => (
            <label key={injury} className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleArrayFieldChange('injuries', injury, e.target.checked)}
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
      ...
      <div className="md:col-span-2">
        <SmartField
          ...
          onChange={(value) => handleFieldChange('previous_gym_experience', value)}
        />
      </div>
    </div>
  );
  ...
  const renderGoalsPreferences = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-gray-700 mb-3">Primary Goals</label>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          {['Morning', 'Afternoon', 'Evening', 'Late Night'].map(time => (
            <label key={time} className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleArrayFieldChange('preferred_workout_times', time, e.target.checked)}
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
  ...
  const renderAccessFeatures = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-gray-700 mb-3">Facility Access</label>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {['Gym Floor', 'Pool', 'Spa', 'Sauna', 'Studio', 'Cardio Room', 'Weight Room', 'Locker Room'].map(facility => (
            <label key={facility} className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={formData.facility_access.includes(facility)}
                onChange={(e) => handleArrayFieldChange('facility_access', facility, e.target.checked)}
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
  ...
  const renderCommunication = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <SmartField
        ...
        onChange={(value) => handleFieldChange('preferred_contact_method', value)}
        options={[
      ...
      <div className="md:col-span-2 space-y-4">
        ...
        <SmartField
          ...
          onChange={(value) => handleFieldChange('progress_tracking_consent', value)}
        />
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-100 text-sm">
                    {mode === 'add' ? 'Create a new member profile' : 'Update member information'}
                  </p>
                </div>
                <button
                  onClick={onClose}
                ...
                {tabs.map(tab => {
                  ...
                  return (
                    ...
                    >
                      {tab.icon}
                      <span>{tab.label}</span>
                      {hasError && <FiAlertCircle className="ml-1 text-red-500 w-4 h-4" aria-label="Tab has errors" />}
                      {!hasError && isComplete && <FiCheckCircle className="ml-1 text-green-500 w-4 h-4" aria-label="Tab complete" />}
                    </button>
                  );
                })}
              </div>
                ...
                >
                  {isSaving ? (
                    ...
                    <>
                      ...
                      <span>{mode === 'add' ? 'Add Member' : 'Save Changes'}</span>
                    </>
                  )}
                </button>
              </div>
