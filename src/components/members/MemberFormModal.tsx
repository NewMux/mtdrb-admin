...
import { FiX, FiUser, FiMail, FiPhone, FiCalendar, FiDollarSign, FiTag, FiFileText, FiShield, FiBriefcase, FiArrowLeft, FiArrowRight, FiCheck, FiSave, FiMapPin, FiTarget, FiHeart } from 'react-icons/fi';
...
import ColorfulModalUI from '../ColorfulModalUI';
...
const schema = z.object({
  ...
  membership_price: z.coerce.number().min(0, 'Price must be positive').optional(),
  ...
  invoice_amount: z.coerce.number().min(0, 'Amount must be positive').optional(),
  invoice_items: z.string().optional(),
  invoice_due_date: z.string().optional(),
  payment_method: z.enum(['card', 'bank_transfer', 'cash', 'digital_wallet', 'cheque']).optional(),
  
  // Smart Health & Fitness Fields
  height: z.coerce.number().min(100, 'Height must be at least 100cm').max(250, 'Height must be under 250cm').optional(),
  weight: z.coerce.number().min(30, 'Weight must be at least 30kg').max(300, 'Weight must be under 300kg').optional(),
  target_weight: z.coerce.number().min(30, 'Target weight must be at least 30kg').max(300, 'Target weight must be under 300kg').optional(),
  ...
  primary_goals: z.array(z.string()).min(1, 'Select at least one fitness goal'),
  goal_timeline: z.enum(['1_month', '3_months', '6_months', '1_year']),
  workout_frequency_goal: z.coerce.number().min(1, 'At least 1 workout per week').max(7, 'Maximum 7 workouts per week'),
  preferred_workout_times: z.array(z.string()).min(1, 'Select at least one preferred time'),
  
  // Staff Management - Membership Setup
  access_level: z.enum(['Basic', 'Premium', 'VIP']),
  membership_duration: z.enum(['1_month', '3_months', '6_months', '1_year', '2_years', 'lifetime']),
  billing_cycle: z.enum(['monthly', 'quarterly', 'semi-annual', 'annual']),
  discount_percentage: z.coerce.number().min(0, 'Discount must be 0 or positive').max(100, 'Discount cannot exceed 100%').optional(),
  discount_amount: z.coerce.number().min(0, 'Discount amount must be positive').optional(),
  auto_renewal: z.boolean(),
  payment_method_preference: z.enum(['card', 'bank_transfer', 'cash', 'digital_wallet', 'cheque']).optional(),
  ...
  if (data.create_invoice) {
    return data.invoice_amount && data.invoice_amount > 0 && data.invoice_due_date;
  }
  return true;
}, {
  ...
  if (data.target_weight && data.weight) {
    ...
    const maxHealthyChange = data.target_weight < data.weight ? 
      timelineWeeks * 0.9 : timelineWeeks * 0.45; // kg conversion
    
    return weightDiff <= maxHealthyChange;
  }
  return true;
}, {
  message: "Goal timeline may be unrealistic for target weight change. Consider a longer timeline.",
  path: ["goal_timeline"]
});
...
const MemberFormModal: React.FC<MemberFormModalProps> = ({ isOpen, onClose, onSaved, member }) => {
  const [session, setSession] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [duplicateStatus, setDuplicateStatus] = useState<{ loading: boolean, message: string | null }>({ loading: false, message: null });
  ...
  const defaultValues: MemberFormData = member
    ? {
      ...
      membership_status: member.membership_status as 'active' | 'trial' | 'cancelled' | 'paused',
      start_date: member.start_date ? member.start_date.split('T')[0] : '',
      end_date: member.end_date ? member.end_date.split('T')[0] : '',
      membership_type: member.membership_type || '',
      membership_price: member.membership_price ? parseFloat(member.membership_price.toString()) : undefined,
      ...
      fitness_level: (member.fitness_level as 'Beginner' | 'Intermediate' | 'Advanced') || 'Beginner',
      medical_conditions: member.medical_conditions || [],
      injuries: member.injuries || [],
      previous_gym_experience: member.previous_gym_experience || false,
      primary_goals: member.primary_goals || [],
      goal_timeline: (member.goal_timeline as '1_month' | '3_months' | '6_months' | '1_year') || '3_months',
      workout_frequency_goal: member.workout_frequency_goal || 3,
      preferred_workout_times: member.preferred_workout_times || [],
      // Staff Management - Membership Setup
      access_level: (member.access_level as 'Basic' | 'Premium' | 'VIP') || 'Basic',
      membership_duration: (member.membership_duration as '1_month' | '3_months' | '6_months' | '1_year' | '2_years' | 'lifetime') || '1_month',
      billing_cycle: (member.billing_cycle as 'Monthly' | 'Quarterly' | 'Semi-Annual' | 'Annual') || 'Monthly',
      ...
      payment_method_preference: (member.payment_method_preference as 'card' | 'bank_transfer' | 'cash' | 'digital_wallet' | 'cheque') || undefined,
      
      // Modern Gym SaaS Features
      access_hours: (member.access_hours as 'Standard' | '24/7' | 'Premium Hours') || 'Standard',
      facility_access: member.facility_access || ['Gym Floor'],
      fitness_tracker_integration: member.fitness_tracker_integration || false,
      ...
      // Communication & Automation Features
      preferred_contact_method: (member.preferred_contact_method as 'email' | 'sms' | 'whatsapp' | 'app') || 'app',
  ...
  const { register, handleSubmit, control, watch, setValue, reset, getValues, formState: { errors } } = useForm<z.infer<typeof schema>>({
  ...
  const BasicInfoStep = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h3 className="text-xl font-semibold text-gray-900 mb-3">Member Information</h3>
        <p className="text-sm text-gray-600">Enter the member's basic details and contact information</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          <label htmlFor="assigned_trainer_id" className="block text-sm font-medium text-gray-700 mb-2">
            Assigned Trainer
          </label>
          <select
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {['VIP', 'Athlete', 'Trial', 'Student', 'Senior', 'Corporate', 'Staff', 'New Member'].map(tag => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label htmlFor="staff_notes" className="block text-sm font-medium text-gray-700 mb-2">
            Staff Notes
          </label>
          <textarea
            id="staff_notes"
  ...
  const HealthFitnessStep = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h3 className="text-xl font-semibold text-gray-900 mb-3">Health & Fitness Profile</h3>
        <p className="text-sm text-gray-600">Record the member's health information and fitness background</p>
      </div>
        ...
        <div>
          <label className="flex items-center space-x-3">
            <input
              ...
              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <span className="text-sm font-medium text-gray-700">Previous Gym Experience</span>
          </label>
          <p className="text-xs text-gray-500 ml-6">Member has experience with gym equipment and routines</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {['Asthma', 'Heart Condition', 'Diabetes', 'Hypertension', 'Back Pain', 'Knee Injury', 'None'].map(condition => (
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {['Shoulder', 'Knee', 'Back', 'Ankle', 'Wrist', 'Hip', 'None'].map(injury => (
  ...
  const GoalsPreferencesStep = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h3 className="text-xl font-semibold text-gray-900 mb-3">Goals & Preferences</h3>
        <p className="text-sm text-gray-600">Configure the member's fitness goals and workout preferences</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {['Weight Loss', 'Muscle Gain', 'Strength Training', 'Cardio Fitness', 'Flexibility', 'Sports Performance'].map((goal) => (
        ...
        {errors.primary_goals && <p className="text-red-500 text-sm mt-1">{errors.primary_goals.message}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          {errors.goal_timeline && <p className="text-red-500 text-sm mt-1">{errors.goal_timeline.message}</p>}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {['Early Morning (6-9 AM)', 'Morning (9-12 PM)', 'Afternoon (12-5 PM)', 'Evening (5-8 PM)', 'Night (8-11 PM)'].map((time) => (
        ...
        {errors.preferred_workout_times && <p className="text-red-500 text-sm mt-1">{errors.preferred_workout_times.message}</p>}
      </div>
    </div>
  );
  ...
  const BillingMembershipStep = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h3 className="text-xl font-semibold text-gray-900 mb-3">Membership Setup & Features</h3>
        <p className="text-sm text-gray-600">Configure access level, duration, pricing, and automation features</p>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-8 rounded-xl border border-blue-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          {[
            { name: 'Basic', features: ['Gym Access', 'Basic Equipment', 'Locker'] },
            { name: 'Premium', features: ['All Basic', 'Group Classes', 'Personal Training Session'] },
            { name: 'VIP', features: ['All Premium', 'Nutrition Consultation', '24/7 Access', 'Priority Support'] }
          ].map(level => (
        ...
        {errors.access_level && <p className="text-red-500 text-sm">{errors.access_level.message}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          {errors.membership_duration && <p className="text-red-500 text-sm mt-1">{errors.membership_duration.message}</p>}
        </div>
        ...
        <div>
          ...
          {errors.billing_cycle && <p className="text-red-500 text-sm mt-1">{errors.billing_cycle.message}</p>}
          <p className="text-xs text-gray-500 mt-1">How often member gets charged</p>
        </div>
      </div>
      ...
      <div className="bg-green-50 p-8 rounded-xl border border-green-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            ...
            {errors.membership_price && <p className="text-red-500 text-sm mt-1">{errors.membership_price.message}</p>}
          ...
          <div>
            ...
            {errors.discount_amount && <p className="text-red-500 text-sm mt-1">{errors.discount_amount.message}</p>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Auto-calculated based on billing cycle</p>
        </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 ml-6">Member's subscription will auto-renew</p>
          </div>
          ...
          <div>
            <label className="flex items-center space-x-3">
              <input
                ...
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="text-sm font-medium text-gray-700">Generate Invoice Now</span>
            </label>
            <p className="text-xs text-gray-500 ml-6">Create invoice immediately upon registration</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <label className="flex items-center space-x-3">
              ...
              />
              <span className="text-sm text-gray-700">Send welcome sequence</span>
            </label>
            ...
            <label className="flex items-center space-x-3">
              <input
                ...
                className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
              />
              <span className="text-sm text-gray-700">Auto-schedule intro session</span>
            </label>
            ...
            <label className="flex items-center space-x-3">
              <input
                ...
                className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
              />
              <span className="text-sm text-gray-700">Send workout reminders</span>
            </label>
            ...
            <label className="flex items-center space-x-3">
              <input
                ...
                className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
              />
              <span className="text-sm text-gray-700">Workout buddy matching</span>
            </label>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <label className="flex items-center space-x-3">
            <input
              ...
              className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
            />
            <span className="text-sm text-gray-700">Allow progress tracking</span>
          </label>
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {['Gym Floor', 'Pool', 'Sauna', 'Group Classes', 'Personal Training Area', 'Kids Zone'].map(facility => (
            ...
            {errors.facility_access && <p className="text-red-500 text-sm mt-1">{errors.facility_access.message}</p>}
          </div>
        </div>
          ...
          <div className="space-y-3">
            <label className="flex items-center space-x-3">
              <input
                ...
                className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
              />
              <span className="text-sm text-gray-700">Corporate Membership</span>
            </label>
        ...
        <div className="border-t pt-4 mt-4">
          <h5 className="font-medium text-gray-900 mb-3">Digital Integration & Tracking</h5>
          <div className="space-y-3">
            <label className="flex items-center space-x-3">
              <input
              ...
              />
              <div>
                <span className="text-sm text-gray-700">Connect fitness tracker</span>
                <p className="text-xs text-gray-500">Sync with Apple Health, Fitbit, Garmin, etc.</p>
              </div>
            </label>
            ...
            <label className="flex items-center space-x-3">
              ...
              />
              <div>
                <span className="text-sm text-gray-700">Body composition tracking</span>
                <p className="text-xs text-gray-500">Regular DEXA scans and measurements</p>
              </div>
            </label>
            ...
            <label className="flex items-center space-x-3">
              ...
              />
              <div>
                <span className="text-sm text-gray-700">Progress photos consent</span>
                <p className="text-xs text-gray-500">Allow trainers to take before/after photos</p>
              </div>
            </label>
          </div>
        </div>
      ...
      <div className="border-t pt-4">
        <label className="flex items-start space-x-3">
          ...
          />
          <div>
            <span className="text-sm font-medium text-gray-700">Member has signed terms and conditions *</span>
            <p className="text-xs text-gray-500">Confirm that the member has read and signed the membership agreement, privacy policy, and liability waiver.</p>
          </div>
        </label>
        {errors.consent_signed && <p className="text-red-500 text-sm mt-1">Staff confirmation required</p>}
      </div>
    </div>
  );
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 1: return <HealthFitnessStep />;
      case 2: return <GoalsPreferencesStep />;
      case 3: return <BillingMembershipStep />;
      default: return <BasicInfoStep />;
    }
  };

  // Staff tool: Calculate end date based on membership duration (not billing cycle)
  const calculateEndDate = useCallback((membershipDuration: string, startDate: string) => {
    ...
    const duration = durations[membershipDuration as keyof typeof durations] || durations['1_month'];
    return start.add(duration.amount, duration.unit as any).format('YYYY-MM-DD');
  }, []);
    ...
    const subscription = watch((value, { name }) => {
      if (name === 'membership_duration' || name === 'start_date') {
        if (value.membership_duration && value.start_date) {
          const calculatedEndDate = calculateEndDate(value.membership_duration, value.start_date);
          setValue('end_date', calculatedEndDate);
        }
      }
    });
  ...
  const validateRequiredFields = useCallback(() => {
    ...
    if (!currentValues.membership_status) errors.push('Membership Status is required');
    ...
    if (isOpen) {
      if (member) {
        ...
        const formattedMember = {
          ...
          start_date: member.start_date ? dayjs(member.start_date).format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD'),
          phone: member.phone || '+973',
          language: (member.language === 'Arabic' ? 'Arabic' : 'English') as 'English' | 'Arabic',
        };
        // Try to auto-detect country code from phone
        const found = COUNTRY_CODES.find(c => formattedMember.phone?.startsWith(c.code));
        if (found) {
          setCountryCode(found.code);
          reset({ 
          ...formattedMember, 
          phone: formattedMember.phone.replace(found.code, ''),
          membership_price: typeof formattedMember.membership_price === 'string' 
  ...
  const checkDuplicate = async (field: 'email' | 'phone', value: string) => {
    ...
    if (data && data.length > 0 && data[0].id !== member?.id) {
      setDuplicateStatus({ loading: false, message: `Member with this ${field} already exists: ${data[0].name}` });
    } else {
  ...
  const onSubmit = async (data: MemberFormData) => {
    ...
    const requiredFields = ['name', 'phone', 'membership_status', 'start_date', 'end_date'];
    const missingFields = requiredFields.filter(field => !data[field as keyof MemberFormData]);
    ...
    const { create_invoice, invoice_amount, invoice_items, invoice_due_date, payment_method, ...memberData } = data;
    const payload = {
      ...memberData,
    ...
    try {
      ...
      if (member) {
        ...
        result = await supabase.from('members').update(payload).eq('id', member.id).select().single();
        ...
        if (create_invoice && savedMember) {
          const invoicePayload = {
            ...
            due_date: invoice_due_date || dayjs().add(30, 'days').format('YYYY-MM-DD'),
          ...
          console.log('Attempting to create invoice with payload:', invoicePayload);
          ...
          if (invoiceError) {
            ...
            toast.error(`Member updated successfully, but invoice creation failed: ${invoiceError.message}`);
          } else {
        ...
        try {
          ...
          if (!tenant_id) {
            console.log('🔍 No tenant_id in metadata, checking memberships table...');
            ...
            if (membershipError) {
              ...
              throw new Error('Could not determine tenant ID: ' + membershipError.message);
            }
          ...
          if (!tenant_id) {
            throw new Error('Could not determine tenant ID. Please log in again.');
          }
        ...
        console.log('📝 Inserting member with payload:', { ...payload, tenant_id });
        
        result = await supabase.from('members').insert({ ...payload, tenant_id }).select().single();
        ...
        if (create_invoice && savedMember) {
          ...
          if (invoiceError) {
            ...
            toast.error(`Member created successfully, but invoice creation failed: ${invoiceError.message}`);
          } else {
        ...
        <SmartModal
          ...
          footer={
            <div className="flex items-center justify-end space-x-3">
              ...
              <SmartButton variant="primary" onClick={handleSubmit} loading={isLoading}>
