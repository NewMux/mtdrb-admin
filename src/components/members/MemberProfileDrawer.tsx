...
export default function MemberProfileDrawer({ member, isOpen, onClose, onEdit, onRefresh }: MemberProfileDrawerProps) {
        ...
        >
          ...
          <div className="relative bg-gradient-to-br from-blue-500 via-blue-600 to-blue-700 px-6 py-6 rounded-tl-3xl flex-shrink-0">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <h1 id="member-profile-title" className="text-white text-xl font-semibold">Member Profile</h1>
              <div className="flex items-center space-x-2">
                <button
                  ...
                  title="Refresh"
                >
                  <FiRotateCw className={`w-5 h-5 ${isLoading ? 'animate-spin' : ''}`} />
                </button>
                <button
                  className="text-white hover:text-blue-100 transition-all duration-200 p-2 rounded-full hover:bg-white/10 active:scale-95 focus:outline-none focus:ring-2 focus:ring-white/50"
            ...
            <div className="text-center">
              ...
              <div className="mb-6">
                <p className="text-blue-100 text-sm font-medium mb-2">
                  {member.membership_status === 'active' ? 'Active Member' :
                   member.membership_status === 'trial' ? 'Trial Member' :
                   member.membership_status === 'suspended' ? 'Suspended Member' :
                   member.membership_status === 'cancelled' ? 'Cancelled Member' :
                   member.membership_status === 'expired' ? 'Expired Member' :
                   'Member'}
                </p>
                <div className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-white/10 backdrop-blur-sm border border-white/20">
                  <div className={`w-2 h-2 rounded-full mr-2 ${
                    member.membership_status === 'active' ? 'bg-emerald-400' :
                    member.membership_status === 'trial' ? 'bg-amber-400' :
                    member.membership_status === 'suspended' ? 'bg-orange-400' :
                    'bg-red-400'
                  }`}></div>
              ...
              <div className="flex justify-center items-center space-x-8">
                <div className="text-center">
                  ...
                  <div className="text-blue-100 text-xs font-medium">Check-ins</div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-blue-100 text-xs font-medium">Attendance</div>
                </div>
              </div>
            </div>
