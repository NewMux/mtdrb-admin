    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
      ...
      {change && (
        ...
        >
          ...
          <span>{change}</span>
          <span className="text-gray-500 ml-1">{t("dashboard.vsLastMonth")}</span>
        </div>
      )}
    </div>
  );
        ...
        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
          <tr>
            <th scope="col" className="px-6 py-3">
              {t("insights.trainerCol")}
            </th>
            <th scope="col" className="px-6 py-3">
              {t("insights.classesCol")}
            </th>
            <th scope="col" className="px-6 py-3">
              {t("insights.totalAttCol")}
            </th>
            <th scope="col" className="px-6 py-3">
              {t("insights.avgAttCol")}
            </th>
          </tr>
        </thead>
        <tbody>
...
const HeatmapChart: React.FC<{ data: HeatmapData }> = ({ data }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
      <h3 className="text-lg font-bold mb-4">{t("insights.peakTimesHeatmap")}</h3>
  ...
  const {
    ...
    if (!classes || !bookings || classes.length === 0) {
      return {
        totalBookings: 0,
        fillRate: "0%",
        mostPopular: t("dashboard.notAvailable"),
        leastPopular: t("dashboard.notAvailable"),
        classPopularity: [],
        trainerPerformance: [],
        heatmapData: {},
      };
    ...
    >((acc, classInfo) => {
      const trainerName = classInfo.trainer || t("insights.unknownTrainer");
      if (!acc[trainerName]) {
        acc[trainerName] = {
          name: trainerName,
          classCount: 0,
    ...
    const popularity = Object.entries(classBookingCounts)
      .map(([classId, count]) => {
        ...
        return {
          name: classInfo?.name || t("insights.unknownClass"),
          attendance: count,
        };
      })
    ...
    return {
      ...
      mostPopular: popularity.length > 0 ? popularity[0].name : t("dashboard.notAvailable"),
      leastPopular:
        popularity.length > 0 ? popularity[popularity.length - 1].name : t("dashboard.notAvailable"),
  ...
  return (
    <div className="p-8 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <MetricCard
          title={t("insights.mostPopularClass")}
          value={mostPopular}
          icon={<FiTrendingUp />}
          change="+12%"
          changeType="up"
        />
        <MetricCard
          title={t("insights.leastPopularClass")}
          value={leastPopular}
          icon={<FiTrendingDown />}
          change="-8%"
          changeType="down"
        />
        <MetricCard
          title={t("insights.averageFillRate")}
          value={fillRate}
          icon={<FiBarChart2 />}
          change="+2%"
          changeType="up"
        />
        <MetricCard
          title={t("insights.totalBookings")}
          value={totalBookings}
          icon={<FiUsers />}
          change="+50"
          changeType="up"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <ChartCard title={t("insights.classPopularity")}>
        ...
        <div className="lg:col-span-3">
          <ChartCard title={t("insights.attendanceTrend")}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={[]}
                margin={{ top: 5, right: 20, left: -10, bottom: 5 }}
