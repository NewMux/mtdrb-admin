...
export const getCampaignRecommendations = async (): Promise<
  ...
  return [
    {
      ...
      description:
        "Increase member referrals with double rewards and social proof",
    ...
    },
    {
      ...
      description:
        "Fill underbooked classes with targeted time-based promotions",
...
export const getAutomationWorkflows = async (): Promise<
  AutomationWorkflow[]
> => {
...
export const generateAICopy = async (
  ...
  return (
    copyTemplates[campaignType as keyof typeof copyTemplates] || [
...
export const calculateExpectedROI = async (
  campaignData: any,
): Promise<number> => {
  ...
  const audienceMultiplier =
    campaignData.audience === "new-members"
      ? 1.2
      : campaignData.audience === "inactive-members"
        ? 1.5
        : 1.0;
  const channelMultiplier = campaignData.channels.length > 2 ? 1.1 : 1.0;
  const discountMultiplier =
    campaignData.discount.type === "percentage"
      ? (100 - campaignData.discount.value) / 100
      : 0.8;
