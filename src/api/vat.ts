...
export const getVatDashboardData = async (
  tenantId: string,
): Promise<VatDashboardData> => {
  try {
    ...
    const currentVatPaid =
      currentTransactions
        ?.filter((t) => t.type === "Expense")
        .reduce((sum, t) => sum + (t.vat_amount || 0), 0) || 0;

    // Calculate previous period totals
    const previousVatCollected =
    ...
    const vatGrowthPercentage =
      previousVatCollected > 0
        ? ((currentVatCollected - previousVatCollected) /
            previousVatCollected) *
          100
        : 0;
    ...
    // Get VAT by country
    const { data: vatByCountry, error: countryError } = await supabase.rpc(
    ...
    const { data: complianceScore, error: complianceError } =
      await supabase.rpc("calculate_vat_compliance_score", {
        p_tenant_id: tenantId,
...
export const getVatReturns = async (
  ...
  try {
    ...
    const { data, error } = await query.order("return_period_start", {
      ascending: false,
    });
...
export const submitVatReturn = async (
  returnId: string,
  userId: string,
): Promise<void> => {
  try {
...
export const runVatComplianceCheck = async (
  ...
  try {
    ...
    if (missingTransactions && missingTransactions.length > 0) {
      issues.push(
        `${missingTransactions.length} invoices missing VAT transactions`,
      );
    }
    ...
    const auditData = {
      ...
      recommendations: [
        ...
        score < 80
          ? "Consider VAT compliance training"
...
export const getVatRateForCountry = async (
  countryCode: string,
): Promise<number> => {
  try {
    const { data, error } = await supabase
      ...
      .or(
        `effective_to.is.null,effective_to.gt.${new Date().toISOString().split("T")[0]}`,
      )
      .single();
...
export const generateVatReport = async (
  ...
  try {
    ...
    const averageVatRate =
      totalTransactions > 0
        ? transactions.reduce((sum, t) => sum + (t.vat_rate || 0), 0) /
          totalTransactions
        : 0;
    ...
    const byRate = transactions.reduce(
      (acc, t) => {
...
export const getVatInsights = async (
  tenantId: string,
): Promise<{
  trends: string[];
  ...
  try {
    ...
    if (dashboardData.vatGrowthPercentage > 20) {
      insights.trends.push(
        "Strong VAT growth - consider reviewing pricing strategy",
      );
    } else if (dashboardData.vatGrowthPercentage < -10) {
      insights.trends.push(
        "Declining VAT collections - investigate business performance",
      );
    }
    ...
    if (dashboardData.complianceScore < 90) {
      insights.recommendations.push(
        "Improve VAT compliance by reviewing overdue returns",
      );
    }

    if (dashboardData.overdueReturns > 0) {
      insights.recommendations.push(
        "Submit overdue VAT returns to avoid penalties",
      );
    }

    if (dashboardData.upcomingDeadlines > 0) {
      insights.recommendations.push(
        "Prepare for upcoming VAT return deadlines",
      );
    }
    ...
    if (dashboardData.overdueReturns > 0) {
      insights.alerts.push(
        `CRITICAL: ${dashboardData.overdueReturns} overdue VAT returns`,
      );
    }
