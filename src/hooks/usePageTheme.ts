...
export const usePageTheme = (pageName: string) => {
  ...
  const getButtonClasses = (
    ...
    const baseClasses =
      "rounded-full px-6 py-3 font-medium transition-all duration-300 flex items-center justify-center space-x-2";
  ...
  const getCardClasses = (
    variant: "default" | "accent" | "hover" = "default",
  ) => {
    const baseClasses =
      "rounded-2xl p-6 bg-white dark:bg-gray-800 border transition-all duration-300";
  ...
  const getTabClasses = (isActive: boolean = false) => {
    const baseClasses =
      "px-4 py-3 text-sm font-medium border-b-2 border-transparent transition-all duration-300";
