...
async function migrateClassSchedules(tenant_id: string, trainer_id: string, trainer_name: string) {
  try {
    ...
    const scheduleEntries = classes.map(cls => ({
      ...
      end_time: cls.end_time || new Date(new Date(cls.start_time).getTime() + 60 * 60 * 1000).toISOString(),
      status: 'scheduled',
      type: 'class',
...
export async function migrateTrainerData() {
  try {
    ...
    if (!tablesExist) {
      throw new Error('Database schema not ready. Please wait for migrations to complete.');
    }
