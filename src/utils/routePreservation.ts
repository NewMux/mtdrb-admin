...
class RoutePreservationManager {
  ...
  private cleanupOldEntries(): void {
    ...
    if (entries.length > MAX_HISTORY_ENTRIES) {
      ...
      this.history = Object.fromEntries(sortedEntries.slice(0, MAX_HISTORY_ENTRIES));
    }
  }
  ...
  saveRouteState(path: string, state: Partial<RouteState>): void {
    ...
    const existingState = this.history[path] || { tab: 'overview', timestamp: Date.now() };
  ...
  // Get sort state
  getSortState(path: string): { field: string; direction: 'asc' | 'desc' } | null {
    const state = this.getRouteState(path);
  ...
  getNavigationHistory(limit: number = 5): Array<{ path: string; timestamp: number }> {
    const entries = Object.entries(this.history)
  ...
  getUsageStats(): {
    ...
    const entries = Object.entries(this.history);
    const mostVisited = entries.sort((a, b) => b[1].timestamp - a[1].timestamp)[0];
